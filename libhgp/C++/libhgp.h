...
/////////////////////////////////////////////////////////////
// LGP utility wrappers (exported by libhgp DLL)
/////////////////////////////////////////////////////////////

// Statistics / combinations
typedef double (*LGP_Variance_Double)(const double* data, int n);
typedef void   (*LGP_IncreaseVector)(int minI, int maxI, Vector1i1 & out);
typedef void   (*LGP_ShuffleVector)(int size, Vector1i1 & out);

// String / number conversions
typedef void   (*LGP_Double2String)(double value, int p, char* out, int outSize);
typedef double (*LGP_String2Double)(const char* str);
typedef bool   (*LGP_StringContain)(const char* str, const char* sub);
typedef void   (*LGP_StringReplace)(const char* source,
...
typedef void   (*LGP_WinGetCurDirectory)(char* out, int outSize);
typedef void   (*LGP_WinGetUserName)(char* out, int outSize);
/////////////////////////////////////////////////////////////
//
...
class HGPPL
{
	...
	HGPPL()
	{
		...
		/////////////////////////////////////////////////////////////
		//
		// LGP utility functions
		LGP_Variance_Double_C    = (LGP_Variance_Double)GetProcAddress(hModule, "LGP_Variance_Double");
		LGP_IncreaseVector_C     = (LGP_IncreaseVector)GetProcAddress(hModule, "LGP_IncreaseVector");
		LGP_ShuffleVector_C      = (LGP_ShuffleVector)GetProcAddress(hModule, "LGP_ShuffleVector");
		LGP_Double2String_C      = (LGP_Double2String)GetProcAddress(hModule, "LGP_Double2String");
		LGP_String2Double_C      = (LGP_String2Double)GetProcAddress(hModule, "LGP_String2Double");
		LGP_StringContain_C      = (LGP_StringContain)GetProcAddress(hModule, "LGP_StringContain");
		LGP_StringReplace_C      = (LGP_StringReplace)GetProcAddress(hModule, "LGP_StringReplace");
		LGP_WinGetCurDirectory_C = (LGP_WinGetCurDirectory)GetProcAddress(hModule, "LGP_WinGetCurDirectory");
		LGP_WinGetUserName_C     = (LGP_WinGetUserName)GetProcAddress(hModule, "LGP_WinGetUserName");
	};
	...
	/////////////////////////////////////////////////////////////
	//
	// LGP utility wrappers
	LGP_Variance_Double      LGP_Variance_Double_C;
	LGP_IncreaseVector       LGP_IncreaseVector_C;
	LGP_ShuffleVector        LGP_ShuffleVector_C;
	LGP_Double2String        LGP_Double2String_C;
	LGP_String2Double        LGP_String2Double_C;
	LGP_StringContain        LGP_StringContain_C;
	LGP_StringReplace        LGP_StringReplace_C;
	LGP_WinGetCurDirectory   LGP_WinGetCurDirectory_C;
	LGP_WinGetUserName       LGP_WinGetUserName_C;
};
#define PL() HGPPL::Inst()

// Convenience C++ wrapper: compute variance from Vector1d1 directly.
// Note: function name differs from typedef LGP_Variance_Double to avoid name clash.
inline double LGP_Variance_Double_from_vec(const Vector1d1& vec)
{
	// Basic input check; mirrors DLL-side guard (n <= 1 -> 0.0).
	if (vec.size() <= 1)
	{
		return 0.0;
	}

	// Ensure function pointer is valid before calling into the DLL.
	if (!PL().LGP_Variance_Double_C)
	{
		return 0.0;
	}

	return PL().LGP_Variance_Double_C(
		vec.data(),
